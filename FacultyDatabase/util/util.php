<?php
	...
	function anystyleIOservice($ch,$citation){
		...
		$result = curl_exec($ch);
		$jsonResult=json_decode($result)[0];
	...
	function  getUserDataObject($userData,$ch){
		...
		for($j='A';$j<='K';$j++){
			...
			switch($j){
				...
				case 'B': $user->setUserName($cellVal);break;
				case 'C': $user->setSequence($cellVal);break;
				case 'D': $user->setTitle($cellVal);break;
				case 'E': $user->setVolume($cellVal);break;
				case 'F': $user->setNumber($cellVal);break;
				case 'G': $user->setPageEnd($cellVal);break;
				case 'H': $user->setPageStart($cellVal);break;
				case 'I': $user->setPMID($cellVal);break;
				case 'J': $user->setEvalYear($cellVal);break;
				case 'K': $user->setPublications($cellVal);
